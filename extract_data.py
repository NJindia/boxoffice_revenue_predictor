...
if __name__ == '__main__':
    ...
    args = sys.argv[1:]
    for arg in args:
        if arg == "--scrape_boxofficemojo": scrape_boxofficemojo = True
        if arg == "--scrape_imdb": scrape_imdb = True
        if arg == "--load_imdb_data": load_imdb_data = True
    ...
    def crawl(process: CrawlerProcess):
        ...
        if scrape_imdb: yield process.crawl(imdb_spider.IMDBSpider, df=box_office_data_temp)
        if reactor.running: reactor.callFromThread(reactor.stop)


    if scrape_imdb or scrape_boxofficemojo:
        crawl(process)
        reactor.run()
